...
**When working with resume_content.md source files:**
If you receive content from a `resume_content.md` file (generated by resume-content-generator):
1. **Read summary_highlights from YAML frontmatter** - The content generator strategically selects 3-4 key metrics
2. **Emphasize the same metrics in Professional Summary** - Ensure your LaTeX summary paragraph mentions these metrics
3. **Bold the highlighted metrics** - Use `\textbf{}` to bold the same metrics (e.g., `\textbf{8+ years}`, `\textbf{1M+ daily requests}`)
4. **Maintain consistency** - Both PDF and web formats should tell the same story with the same emphasis

Example: If summary_highlights includes `"8+ Years"`, `"1M+ Requests"`, and `"99.9% Uptime"`, your Professional Summary should read:
> "Senior ML Engineer with \textbf{8+ years} building production systems serving \textbf{1M+ daily requests} with \textbf{99.9% uptime}..."
